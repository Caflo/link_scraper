...
def link_scrape(url, base_url, verbose=False):
    result = dict()
    list = []

    # collecting statistics
    start_time = timeit.default_timer()
    result['data'] = analyze_anchors(url, base_url, result=list, verbose=verbose)
...
#    test_curl("localhosts")
    result = link_scrape("localhost", "localhost", verbose=True)
    # print('\n'.join(result['data']))